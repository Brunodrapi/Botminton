...
// Les adresses portent une empreinte de contenu (voir tools/stamp.mjs) : on l'ignore pour lire les fichiers.
html = html.replace(/<link rel="stylesheet" href="style\.css(?:\?v=[0-9a-f]+)?">/, () => `<style>\n${readFileSync('style.css', 'utf8')}\n</style>`);
html = html.replace('<link rel="manifest" href="manifest.webmanifest">\n', '');
html = html.replace('<link rel="apple-touch-icon" href="icon.svg">\n', '');
html = html.replace(/<script src="(src\/[^"?]+)(?:\?v=[0-9a-f]+)?"><\/script>/g, (_, f) => `<script>\n${readFileSync(f, 'utf8')}\n</script>`);
