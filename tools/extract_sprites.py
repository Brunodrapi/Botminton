...
SRC = 'assets/rg-b1-sheet.png'
OUT_PNG = 'assets/rg-b1.png'
OUT_JSON = 'assets/rg-b1.json'
DEBUG = len(sys.argv) > 1 and sys.argv[1] == '--debug'
BODY_H = 32          # hauteur cible (px du tampon) du robot de face

im = np.array(Image.open(SRC).convert('RGB')).astype(np.int16)
H, W, _ = im.shape

# bandes (y0, y1) et noms des cellules, de gauche à droite
BANDS = [
    ((178, 392, 0, 1536), ['face', 'back', 'side_l', 'side_r', 'idle', 'walk1', 'walk2', 'run1', 'run2']),
    ((478, 712, 0, 1536), ['prep', 'hit_low', 'hit_mid', 'hit_high', 'follow', 'smash_prep', 'jump', 'smash_hit', 'land']),
    ((786, 965, 0, 870), ['serve', 'receive', 'miss', 'win', 'lose']),
]
OBJ = (884, 770, 1150, 965)   # panneau volants (x0, y0, x1, y1)
...
def split_cells(band):
    ...
    for x in range(W + 1):
        ...
        if not on and start is not None:
            if x - start > 25: cells.append((start, x))
            start = None
...
for band, names in BANDS:
    ...
    if len(cells) != len(names):
        print('bande', band, ':', len(cells), 'cellules trouvées pour', len(names), 'noms', cells)
...
# facteur unique : la vue de face fait BODY_H px
f = sprites['face'][1].shape[0] / BODY_H
print('facteur de réduction', round(f, 2))
...
seed = np.array([[20, 22, 18], [46, 78, 62], [70, 110, 88], [200, 100, 50], [235, 228, 205], [150, 150, 140], [110, 60, 30], [90, 140, 110], [240, 170, 110], [60, 60, 56]], dtype=np.float32)
pal = kmeans(allpx.astype(np.float32), len(seed), seed=seed)
...
json.dump(entries, open(OUT_JSON, 'w'), indent=0)
print('atlas', AW, 'x', AH, '→', OUT_PNG)
for n, e in entries.items(): print(f"  {n:11s} {e['w']:3d}x{e['h']:3d} ancre {e['ax']}")

if DEBUG:
    big = Image.fromarray(atlas).resize((AW * 4, AH * 4), Image.NEAREST)
    bg = Image.new('RGBA', big.size, (60, 120, 90, 255)); bg.alpha_composite(big)
    bg.save('/tmp/claude-0/-home-user-Botminton/c2453339-d317-59ec-a813-e906198af948/scratchpad/atlas_debug.png')

# ---------------------------------------------------------------- module JS embarqué (aucun chargement réseau)
import base64, io
buf = io.BytesIO(); Image.fromarray(atlas).save(buf, format='PNG')
b64 = base64.b64encode(buf.getvalue()).decode('ascii')
js = "/* Généré par tools/extract_sprites.py à partir de assets/rg-b1-sheet.png — ne pas éditer à la main. */\n"
js += "window.SPRITE_ATLAS = {\n  frames: " + json.dumps(entries, separators=(',', ':')) + ",\n"
js += "  png: 'data:image/png;base64," + b64 + "'\n};\n"
